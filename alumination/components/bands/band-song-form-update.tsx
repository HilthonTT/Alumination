...
import { Band, BandSong } from "@prisma/client";
import {
  Form,
  FormControl,
  FormDescription,
...
interface BandSongFormProps {
  band: Band;
  initialData: BandSong;
}

export const BandSongFormUpdate = ({
  band,
  initialData,
}: BandSongFormProps) => {
  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    ...
    try {
      await axios.patch(
        `/api/bands/${band?.id}/songs/${initialData?.id}`,
        values
      );
      ...
      router.refresh();
      form.reset();
      window.location.href = `/bands/${band?.id}/songs`;
    } catch (error) {
      console.log(error);
  ...
  return (
    <>
      <PageHeader title="Update a song for your band" />
      <Form {...form}>
